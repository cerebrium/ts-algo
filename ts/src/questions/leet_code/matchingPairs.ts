...
export function matchingPairs(s: string, t: string): number {
  let pairs: number = 0;

  const lookupMap: Map<string, Map<string, number>> = new Map(); // <has, Map<looking-for, number>>
  const availableMatches: Array<[string, string]> = []; // [has, looking-for]

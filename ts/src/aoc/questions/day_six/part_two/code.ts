...
export class DaySixPartTwo {
  public distinct_loops = 0;
  ...
  private _obstacles_hit: ObstaclesHit = {
    left: [],
    right: [],
    down: [],
    up: [],
  };
  private _starting_location: [number, number] | undefined;
  private _possible_axis_loops: Array<[number, number]> = [];

  constructor(input: Array<number[]>) {
    this._input = input;
  }

  private _day_six_part_one() {
  ...
  public day_six_part_two(): number {
    ...
    for (let i = 0; i < this._obstacles_hit.left.length; i++) {
      const is_loopable = this._has_left_and_right(
        this._obstacles_hit.left[i],
        'l'
      );
  ...
  private _has_left_and_right(
    coord: [number, number],
    dir: 'l' | 'u' | 'r' | 'd'
  ) {
    let has_both: Array<boolean | string> = [false, false];
    const [x, y] = coord;
    switch (dir) {
      case 'u':
        for (let i = 0; i < this._obstacles_hit['right'].length; i++) {

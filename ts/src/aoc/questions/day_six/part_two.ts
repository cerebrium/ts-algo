...
export function day_six_part_two(input: string[]) {
  const dirs = [
    [-1, 0], // up
    [0, 1], // right
    [1, 0], // down
  ...
  let curr_coord: [number, number] = [0, 0];
  const rows_map: Map<number, number[]> = new Map();
  const columns_map: Map<number, number[]> = new Map();
  ...
  while (true) {
    ...
    switch (next_map_val) {
      ...
      case '.':
        if (
          check_if_can_add_wall(
            [x + dirs[curr_direction][0], y + dirs[curr_direction][1]],
            rows_map,
            columns_map,
            curr_direction,
            input
...
function check_if_can_add_wall(
  curr_coord: [number, number],
  rows_map: Map<number, number[]>,

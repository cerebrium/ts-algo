...
export class DayTen {
  data: Array<number[]> = [];
  private visited_terminations: Map<string, Array<string>> = new Map();
  ...
  private final_sum: number = 0;
  init(data: Array<number[]>) {
    this.data = data;
  }
  ...
  private walk(
    ...
    if (incline === 9 && found_incline === 9) {
      const found_locations = this.visited_terminations.get(
        `${coord[0]}${coord[1]}`
      );
    ...
    for (
      let new_coord_idx = 0;
      new_coord_idx < new_coords.length;
      new_coord_idx++
    ) {

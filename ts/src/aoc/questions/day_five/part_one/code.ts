...
export class DayFive {
  private ajacency_list: Map<number, number[]>;
  private are_cycles: boolean;
  private updates: number[][];

  public handle_day_five: () => number;
  ...
  private top_sort(): number {
    ...
    const lookup_for_ordering: Map<number, number> = new Map();
    ...
    for (const update of this.updates) {
      const pot_num_to_add = this.walk_orders_top_sort(
        lookup_for_ordering,
        update
      );
  ...
  private walk(current_node: number, visited: Set<number>): boolean {
    // pre

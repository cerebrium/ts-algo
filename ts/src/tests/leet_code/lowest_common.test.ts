...
} from '../../questions/leet_code/lowest_common_ancestor_b_tree';

function makeTreeNodes(nums: Array<number | null>, p: number, q: number) {
  let currLength = 1;
  let idx = 0;
  const rows = [];
  let qNode = null;
...
test('lowest_common_ancestor', () => {
  ...
  const val = lowestCommonAncestor(root, pNode, qNode);

import {
  ListNode,
  addTwoNumbers,
} from '../../questions/leet_code/add_two_numbers';

test('add_two_numbers', () => {
  // Input: l1 = [2,4,3], l2 = [5,6,4]
  // Output: [7,0,8]

  const node_one = new ListNode(2, new ListNode(4, new ListNode(3)));
  const node_two = new ListNode(5, new ListNode(6, new ListNode(4)));

  const answer = addTwoNumbers(node_one, node_two);
...
test('add_two_numbers_remainder', () => {
  ...
  const node_one = new ListNode(input_one[0]);
  let curr_node_one = node_one;

  for (let i = 1; i < input_one.length; i++) {
    curr_node_one.next = new ListNode(input_one[i]);
    curr_node_one = curr_node_one.next;
  }

  const node_two = new ListNode(input_two[0]);
  let curr_node_two = node_two;

  for (let i = 1; i < input_two.length; i++) {
    curr_node_two.next = new ListNode(input_two[i]);

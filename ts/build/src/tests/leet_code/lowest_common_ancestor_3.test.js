...
const lowest_common_ancestor_3_1 = require("../../questions/leet_code/lowest_common_ancestor_3");
function makeTree(nums, p, q) {
    let currLength = 1;
    let idx = 0;
    const rows = [];
    let qNode = null;
    ...
    for (let x = 0; x < rows.length; x++) {
        ...
        for (let i = 0; i < row.length; i++) {
            ...
            const newNode = new lowest_common_ancestor_3_1._Node(row[i]);
            if (row[i] === p) {
                pNode = newNode;
            }
            if (row[i] === q) {
...
test('lowest_common_ancestor_3', () => {
    ...
    const ancestor = (0, lowest_common_ancestor_3_1.lowestCommonAncestorThree)(p, q);
...
//# sourceMappingURL=lowest_common_ancestor_3.test.js.map
...
function day_four_part_one(input) {
    ...
    for (let i = 0; i < input.length; i++) {
        for (let x = 0; x < input[i].length; x++) {
            if (input[i][x] === 'X') {
                const count = _depth_first_xmas([i, x], input, 0, input[i].length, input.length, 'M', -1);
                if (count) {
                    found_xmas += count;
                }
            }
 ...
 *
 */
function _depth_first_xmas(node, graph, node_count, width, height, next_value, dir_idx) {
    ...
    if (dir_idx >= 0) {
        ...
        if (graph[x][y] === next_value) {
            ...
            return _depth_first_xmas([x, y], graph, node_count, width, height, next_values[next_values.indexOf(graph[x][y]) + 1], dir_idx);
        }
        return;
    }
    ...
    for (let i = 0; i < possible_directions.length; i++) {
        const found_node = _depth_first_xmas(node, graph, node_count, width, height, 'M', i);
        if (found_node) {
